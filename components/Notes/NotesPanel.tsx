...
import {
  StickyNote, Plus, Trash2, Calendar, Sparkles, X
} from "lucide-react";
...
interface NotesPanelProps {
  ...
  accentColor: string;
  onAddNote: (text: string, range?: DateRange, targetMonth?: string) => void;
...
export default function NotesPanel({
  ...
  const [activeFilter, setActiveFilter] = useState<"all" | "range" | "general">("all");
  ...
  useEffect(() => {
    if (hasRange) setAttachToRange(true);
    else setAttachToRange(false);
  }, [hasRange]);
  ...
  return (
    <div className="flex flex-col h-full gap-4 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2">
        ...
        >
          <textarea
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && e.metaKey) handleSubmit(e as never);
            }}
          />
      ...
      >
        {(["all", "range", "general"] as const).map((tab) => (
          <button
            key={tab}
